...
static int wm8960_configure_clocking(struct snd_soc_component *component)
{
	...
	if (!(iface1 & (1<<6))) {
		dev_dbg(component->dev,
			"Codec is slave mode, configuring dividers from BCLK\n");
		/* In slave mode Pi drives BCLK/LRCLK. WM8960 still needs
		 * internal dividers configured. Use BCLK as freq_in for PLL. */
		if (!wm8960->freq_in)
			wm8960->freq_in = wm8960->bclk;
	}

	if (wm8960->clk_id != WM8960_SYSCLK_MCLK && !wm8960->freq_in) {
		/* No MCLK provided - use BCLK as PLL input (Pi I2S slave mode) */
		dev_dbg(component->dev, "No MCLK, using BCLK as PLL input\n");
		wm8960->freq_in = wm8960->bclk;
	}
					...
					unsigned int freq, int dir)
{
	...
	/* If no external clock frequency provided, clear freq_in so
	 * configure_clocking will fall back to using BCLK as PLL input */
	if (freq == 0)
		wm8960->freq_in = 0;
...
MODULE_DESCRIPTION("ASoC WM8960 driver - BirdDog PLL mode (no external MCLK)");
