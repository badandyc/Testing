...
curl -fsSL "https://raw.githubusercontent.com/badandyc/Testing/master/bdm/bdm_initial_setup.sh?$(date +%s)" -o bdm_initial_setup && \
sudo bash bdm_initial_setup

Step 2: Build AP + Networking \
rm -f bdm_AP_setup && \
curl -fsSL "https://raw.githubusercontent.com/badandyc/Testing/master/bdm/bdm_AP_setup.sh?$(date +%s)" -o bdm_AP_setup && \
sudo bash bdm_AP_setup

Step 3: Install MediaMTX \
rm -f bdm_mediamtx_setup && \
curl -fsSL "https://raw.githubusercontent.com/badandyc/Testing/master/bdm/bdm_mediamtx_setup.sh?$(date +%s)" -o bdm_mediamtx_setup && \
sudo bash bdm_mediamtx_setup

Step 4: Install Nginx \
rm -f bdm_web_setup.sh && \
curl -fsSL "https://raw.githubusercontent.com/badandyc/Testing/master/bdm/bdm_web_setup.sh?$(date +%s)" -o bdm_web_setup.sh && \
